...
class DirectorDAO:
    ...
    def insertar(self, director):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO director (nombre, apellido, nacionalidad) VALUES (%s, %s, %s) RETURNING id",
            (director.nombre, director.apellido, director.nacionalidad)
        )
        director.id = cursor.fetchone()["id"]
        conn.commit()
        conn.close()
    ...
    def buscar_por_id(self, director_id):
        conn = obtener_conexion()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM director WHERE id = %s", (director_id,))
    ...
    def buscar_por_nombre_apellido(self, nombre, apellido):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM director WHERE nombre = %s AND apellido = %s",
            (nombre, apellido)
        )
        fila = cursor.fetchone()
        conn.close()
    ...
    def actualizar(self, director_id, nombre=None, apellido=None, nacionalidad=None):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE director SET nombre = %s, apellido = %s, nacionalidad = %s WHERE id=%s",
    ...
    def total(self):
        conn = obtener_conexion()
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) AS total FROM director")
